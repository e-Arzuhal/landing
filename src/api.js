...
export async function getLandingStats() {
  const res = await fetch(`${BASE_URL}/api/landing/stats`)
  if (!res.ok) throw new Error('Stats yüklenemedi')
  return res.json()
}

export async function submitDemoRequest(data) {
  const res = await fetch(`${BASE_URL}/api/landing/demo-request`, {
    method: 'POST',
...
export async function subscribeNewsletter(email) {
  const res = await fetch(`${BASE_URL}/api/landing/newsletter`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
